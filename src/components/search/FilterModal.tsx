    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          <div>
            <p className="mb-1 text-sm font-medium">{t('filters.category')}</p>
            <Select value={values.category} onValueChange={v => setValues({ ...values, category: v, subcategory: '' })}>
              <SelectTrigger>
                <SelectValue placeholder={t('filters.placeholder_category')} />
              </SelectTrigger>
              <SelectContent>
                {categories.map(cat => (
          ...
          {subcategories.length > 0 && (
            <div>
              <p className="mb-1 text-sm font-medium">{t('filters.subcategory')}</p>
              <Select value={values.subcategory} onValueChange={v => setValues({ ...values, subcategory: v })}>
                <SelectTrigger>
                  <SelectValue placeholder={t('filters.placeholder_subcategory')} />
                </SelectTrigger>
                <SelectContent>
          ...
          <div>
            <p className="mb-1 text-sm font-medium">{t('filters.max_price')}</p>
            <Input
              type="number"
          ...
          <div className="flex justify-end pt-2 space-x-2 rtl:space-x-reverse">
            <Button variant="outline" onClick={onClose}>{t('filters.cancel')}</Button>
